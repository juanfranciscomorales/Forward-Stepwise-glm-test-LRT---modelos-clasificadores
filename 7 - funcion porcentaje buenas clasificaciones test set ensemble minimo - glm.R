...
clasificaciones.test.set.ensemble.minimo.glm <- function (test.set = "Dtest.xlsx",cant.modelos = 10, x = tabla.AUC.ordenadas.glm){
        ...
        resultado.final <- list("punto de corte", resultados.ensemble.minimo[[4]], "% bien clasificados test set", porcentaje.bien.clasificados,"Classification Matrix", tabla.bien.mal.clasificados) ## lista con todos los resultados que quiero que aparezcan cuando aplico la funcion
...
clasificaciones.test.set.ensemble.minimo.glm(test.set = "Dtest.xlsx",cant.modelos = 10, x = tabla.AUC.ordenadas.glm)

clasificaciones.test.set.ensemble.minimo.glm(test.set = "Dtest.xlsx",cant.modelos = 10, x = tabla.sensibilidad.ordenadas)
