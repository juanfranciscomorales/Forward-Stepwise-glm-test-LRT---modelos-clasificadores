...
clasificaciones.base.datos.ensemble.voting.glm <- function (base.datos = "Dtest.xlsx",cant.modelos = 10,fraccion.votos = 0.5, x = tabla.AUC.ordenadas.glm){
  ...
  predicciones.ensemble.voto.mayoria.base.datos <- ifelse(votos > cant.modelos*fraccion.votos, yes=1,no=0 ) ## lo que hago es que me diga que si mas de la mitad me lo clasifica como +1 entonces es +1, sino 0
  ...
  tabla.predicciones.ensemble$NOMBRE <- df.base.datos[, "NAME"] #agrego la columna de los nombres de cada compuesto
...
tabla.predicciones.base.datos <- clasificaciones.base.datos.ensemble.voting.glm(base.datos  = "Dtest.xlsx",cant.modelos = 10,fraccion.votos = 0.5, x = tabla.AUC.ordenadas.glm) ## si quiero que sea por AUC

tabla.predicciones.base.datos <- clasificaciones.base.datos.ensemble.voting.glm(base.datos  = "Dtest.xlsx",cant.modelos = 10,fraccion.votos = 0.5, x = tabla.sensibilidad.ordenadas) ## si quiero que sea por modelos con mayor sensibilidad
