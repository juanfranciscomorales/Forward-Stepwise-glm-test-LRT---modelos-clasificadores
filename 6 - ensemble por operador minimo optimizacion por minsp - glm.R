...
combinacion.modelos.minimo <- function (x = tabla.AUC.ordenadas.glm, cant.modelos = 10, minSp = 0.98) { #funcion donde x es el resultado de la funcion del script de curvas ROC.R y cant.modelos es la cant de modelos que quiero combinar
        ...
        controles <- control.cutpoints(valueSp= minSp) 
        
        punto.corte <- OptimalCutpoints::optimal.cutpoints (X = "minimo",status="clase",tag.healthy=0, data=df,methods="MinValueSp", control = controles) ## optimizo el punto de corte usando la funcion de maxima especificidad
        
        punto.corte2 <- data.frame(OptimalCutpoints::summary.optimal.cutpoints(punto.corte)$p.table$Global$MinValueSp)["cutoff",] ## extraigo el valor de punto de corte
...
resultados.ensemble.minimo <- combinacion.modelos.minimo(x = tabla.AUC.ordenadas.glm, cant.modelos = 10, minSp = 0.98)

resultados.ensemble.minimo <- combinacion.modelos.minimo(x = tabla.sensibilidad.ordenadas, cant.modelos = 10, minSp = 0.98)